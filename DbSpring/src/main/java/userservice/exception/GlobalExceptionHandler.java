...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleBadRequest(InvalidInputException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ApiErrorResponse> handleValidation(
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, Messages.VALIDATION_FAILED, request, details);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiErrorResponse> handleUnexpected(Exception ex, HttpServletRequest request) {
        log.error("Unhandled exception for path {}", request.getRequestURI(), ex);
        return buildResponse(HttpStatus.INTERNAL_SERVER_ERROR, Messages.INTERNAL_SERVER_ERROR, request, List.of());
    }
